  ...
  -->
    <div>
      <div class="q-pa-sm q-gutter-sm">
        <div class="text-h6 text-center bg-grey-6">{{ id_row }} Organoid Attributes</div>
              ...
              <q-dialog v-model="show_dialog">
                <q-card  text-center>
                  <q-card-section>
                    ...
                    <div class="text-h6 text-center bg-grey-5">Biospecimen Organoid {{ id_row }} Attributes</div>
                  </q-card-section>
                  ...
                  <q-card-section>
                    ...
                    <div class=" ">
                      <!-- <q-input v-model="editedItem.uuid" hidden/> -->
                      ...
                      <q-select class="fit q-mb-sm"  v-model="editedItem.source" label="source" :options="optionsSource" stack-label
                        ...
                        <q-input outlined  v-model="editedItem.lot_no"  stack-label
                        :rules="[val => val && val.length > 0 || 'Please type something']"   dense label="Lot Number"/><q-space />
                        ...
                        <q-input outlined  v-model="editedItem.hyperlink"  stack-label
...
<!--   
  ...
  const columns = [
    ...
    // { name: 'third', align: 'center', label: 'passage', field: 'passage', sortable: true },
    { name: 'four', align: 'center', label: 'organism', field: 'organism', sortable: true },
    { name: 'six', align: 'center', label: 'Subtype', field: 'subtype', sortable: true },
    { name: 'seven', align: 'center', label: 'Lot Number', field: 'lot_no', sortable: true },
    { name: 'eight', align: 'center', label: 'Description', field: 'description', sortable: true },
...
for (let i = 1; i <= 100; i++) {
  ...
  export default {
    ...
    methods: {
      ...
      async addRow() {
        ...
        if (this.editedIndex > -1) {
          Object.assign(oldRow, updatedRow) && API("auth.updateBiospecimen_org_pdx", { id1: id1,type: type1,type_id:type_id1, source: source1,organism:organism1,subtype:subtype1,lot_no:lot_no1,description:description1,hyperlink: hyperlink1});
        } else {
          if ( source1  && organism1 && subtype1 && lot_no1 && description1 && hyperlink1 ) {
            
            this.data.push(this.editedItem) && API("auth.create_obj_org_pdx", { type: type1,type_id:type_id1, source: source1,organism:organism1,subtype:subtype1,lot_no:lot_no1,description:description1,hyperlink: hyperlink1});
          }
          else {
            this.$q.notify({
      ...
      async deleteItem(item) {
        ...
        this.data.splice(index, 1) && await API('auth.deleteBiospecimen_pdx_org', { id });
      },
      async editItem(item) {
    ...
    // API("auth.createArirports", { name:"DeliAir", latLong:"-873.45",noOfRunWay:"3",typesOfAircraft:"full",runWayConstType:"simple",AtcFacility:"fine",refuling:"ok" });
      ...
      key=this.id_row
      // console.log(key)
      API('auth.bioSpecimen_detials_pdx_org',{key}).then(res => {
        this.data = res
