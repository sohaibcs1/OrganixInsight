  ...
  -->
    <div>
      <div class="q-pa-sm q-gutter-sm">
        <div class="text-h6 text-center bg-grey-6">{{ id_row }} Attributes</div>
              ...
              <q-dialog v-model="show_dialog">
                <q-card style="width:800px;max-width:800px;background-color: #F5F5F5" text-center>
                  <q-card-section>
                    ...
                    <div class="text-h6 text-center bg-grey-5">Perturbagens Virus- {{ id_row }} Attributes</div>
                  </q-card-section>
...
<!--
          ...
          <template v-slot:body="props">
            <br>
                        <q-input  type="text" outlined v-model="modelDosage" stack-label class="fit" :rules="[val => val && val.length > 0 || 'Please type something']"   dense label="Gene ID"/>
...
for (let i = 1; i <= 100; i++) {
  ...
  export default {
    ...
    modelDosage(newValue){
      this.saveDosage(newValue);
    }
    ,
    ...
    methods: {
      ...
      saveLink(link) {
        const link1=link
        const type_id1=this.id_row
        // const dosage1=this.dosage
        // console.log(dosage1,link1)
        API("auth.updateImmunofluorescent_nc_one_DSR_time", { time: link1,type_id:type_id1});
        
      },
       saveDosage(dosage) {
        const dosage1=dosage
        const type_id1=this.id_row
        // const dosage1=this.dosage
        // console.log(dosage1)
        API("auth.updateImmunofluorescent_nc_one_DSR_dosage", { type_id:type_id1,dosage: dosage1});
      ...
      async addRow() {
        ...
        const dosage1= this.editedItem.dosage
        ...
        if (this.editedIndex > -1) {
          Object.assign(oldRow, updatedRow) && API("auth.updatePerturbagens_DSR", { id1: id1,type: type1,type_id:type_id1, concentration: concentration1,concentration_unit:concentration_unit1,dosage:dosage1});
        } else {
          if ( concentration1 && concentration_unit1 ) {

            this.data.push(this.editedItem) && API("auth.create_obj_DSR", { type: type1,type_id:type_id1, concentration: concentration1,dosage:dosage1});
          }
          else {
            this.$q.notify({
