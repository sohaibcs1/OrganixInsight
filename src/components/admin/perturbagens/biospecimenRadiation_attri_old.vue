  ...
  -->
    <div>
      <div class="q-pa-sm q-gutter-sm">
        <div class="text-h6 text-center bg-grey-6">{{ id_row }} Perturbagens Radiation Attributes</div>
              ...
              <q-dialog v-model="show_dialog">
                <q-card style="width:800px;max-width:800px;background-color: #F5F5F5" text-center>
                  <q-card-section>
                    ...
                    <div class="text-h6 text-center bg-grey-5">Perturbagens Radiation- {{ id_row }} Attributes</div>
                  </q-card-section>
...
<!-- 
                        ...
                        <q-input outlined  v-model="editedItem.dosage"  stack-label
                        :rules="[val => val && val.length > 0 || 'Please type something']"   dense label="Enter Dosage"/><q-space />

                        <q-input outlined  v-model="editedItem.time"  stack-label
                        :rules="[val => val && val.length > 0 || 'Please type something']"   dense label="Enter Time"/><q-space />
                        <q-select outlined v-model="editedItem.time_unit" input-debounce="0"  @filter="filterSource" use-input multiple :options="optionsSource"   counter max-values="1"
                        hint="Max 1 selection" label="Select Time Unit:" stack-label dense  /><q-space />
  ...
  const columns = [
    ...
    { name: 'one', align: 'center', label: 'Time Unit', field: 'time_unit', sortable: true },
    { name: 'two', align: 'center', label: 'Dosage', field: 'dosage', sortable: true },
  ...
  const stringOptionsSource= ['Minutes','Hours','Days']
      ...
      async addRow() {
        ...
        if (this.editedIndex > -1) {
          ...
          if ( concentration1 && concentration_unit1 ) {
            ...
            this.data.push(this.editedItem) && API("auth.create_obj_DSR", { type: type1,type_id:type_id1, concentration: concentration1,concentration_unit:concentration_unit1});
          }
          else {
            this.$q.notify({
