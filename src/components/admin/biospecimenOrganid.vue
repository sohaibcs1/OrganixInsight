<template>
  ...
  <div>
    <div class="q-pa-sm q-gutter-sm">
      <div class="text-center text-black text-h6">Organoid</div>
  ...
  {{ props.row.type_id }}</q-badge>
    ...
    <q-dialog v-model="show_dialog">
      <q-card style="width: 230px; max-width: 230px; background-color: #F5F5F5">
        <q-card-section>
          <div class="text-h6 text-center bg-grey-5">Add Organoid</div>
        </q-card-section>
    ...
    <q-dialog v-model="show_dialog_attributes">
      <q-card
        style="width: 1500px; max-width: 1500px; background-color: #F5F5F5"
      >
        <!-- <q-card-section>
...
import biospecimenOrganid_attri from "src/components/admin/biospecimenOrganid_attri";
...
export default {
  ...
  components: { biospecimenOrganid_attri },
  methods: {
    refreshPage() {
    ...
    async addRow() {
      ...
      if (this.editedIndex > -1) {
        ...
        // this.data[editedIndex1] = updatedRow;
          API("auth.updateBiospecimen_org_pdx", { id1: id1, type_id: type_id1 });
      } else {
        if (type_id1) {
          this.data.push(this.editedItem) ;
          // console.log("Added row:", this.editedItem); // Debugging line: Check the added row data

          await API("auth.create_obj_org_pdx", { type_id: type_id1, type:type1 });
        } else {
          this.$q.notify({
        ...
        API("auth.bioSpecimen_detials_organid").then((res) => {
      this.data = res;
    });
    ...
    async deleteItem(item) {
      ...
      confirm("Are you sure you want to Delete this user?") &&
        ...
        (await API("auth.deleteBiospecimen_pdx_org", { id }));
    },
    async editItem(item) {
