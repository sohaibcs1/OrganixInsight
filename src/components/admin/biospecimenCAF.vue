<template>
  ...
  <div>
    <div class="q-pa-sm q-gutter-sm">
      <div class="text-center text-black text-h6">CAF</div>
...
<!--
    ...
    <q-dialog v-model="show_dialog">
      <q-card style="width: 230px; max-width: 230px; background-color: #F5F5F5">
        <q-card-section>
          <div class="text-h6 text-center bg-grey-5">Add CAF</div>
        </q-card-section>
    ...
    <q-dialog v-model="show_dialog_attributes">
      <q-card
        style="width: 2000px; max-width: 2000px; background-color: #F5F5F5"
      >
        <!-- <q-card-section>
...
import biospecimenCAF_attri from "src/components/admin/biospecimenCAF_attri";
...
export default {
  ...
  components: { biospecimenCAF_attri },
  methods: {
    refreshPage() {
    ...
    async addRow() {
      ...
      if (this.editedIndex > -1) {
        ...
        API("auth.bioSpecimenDetials_distinct_type_id_CAF").then((res) => {
      this.data = res;
    });
