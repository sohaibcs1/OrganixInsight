<template>
  ...
  <div>
    <div class="q-pa-sm q-gutter-sm">
      <div class="text-center text-black text-h6">Tumor</div>
    ...
    <q-dialog v-model="show_dialog">
      <q-card style="width: 230px; max-width: 230px; background-color: #F5F5F5">
        <q-card-section>
          <div class="text-h6 text-center bg-grey-5">Add Tumor</div>
        </q-card-section>
...
import biospecimenTumor_attri from "src/components/admin/biospecimenTumor_attri";
...
export default {
  ...
  components: { biospecimenTumor_attri },
  methods: {
    async addRow() {
      ...
      if (this.editedIndex > -1) {
        ...
        // this.data[editedIndex1] = updatedRow;
          API("auth.updateBiospecimen_tumor", { id1: id1, type_id: type_id1 });
      } else {
        if (type_id1) {
          this.data.push(this.editedItem) ;
          // console.log("Added row:", this.editedItem); // Debugging line: Check the added row data

          await API("auth.create_obj_tumor", { type_id: type_id1, type:type1 });
        } else {
          this.$q.notify({
      ...
      API("auth.bioSpecimen_detials_tumor").then((res) => {
      this.data = res;
    });
    ...
    async deleteItem(item) {
      ...
      confirm("Are you sure you want to Delete this user?") &&
        ...
        (await API("auth.deleteBiospecimen_tumor", { id }));
    },
    async editItem(item) {
