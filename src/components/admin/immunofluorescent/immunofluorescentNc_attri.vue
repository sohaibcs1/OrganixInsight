  ...
  -->
    <div>
      <div class="q-pa-sm q-gutter-sm">
        <div class="text-h6 text-center bg-grey-6">{{ id_row }} Nuclear Counterstain Attributes</div>
              ...
              <q-dialog v-model="show_dialog">
                <q-card style="width:800px;max-width:800px;background-color: #F5F5F5" text-center>
                  <q-card-section>
                    ...
                    <div class="text-h6 text-center bg-grey-5">Nuclear Counterstain- {{ id_row }} Attributes</div>
                  </q-card-section>
...
<!--
                        ...
                        <q-input type="textarea" class="fit" outlined  v-model="editedItem.location"  stack-label
          ...
          <template v-slot:body="props">
            ...
            <q-tr :props="props">
              ...
              <q-td key="desc" :props="props">
                ...
                <q-select outlined   v-model="OptionSelections1model" use-input  :options="OptionSelections1"   counter max-values="1"
                        label="Select:" stack-label dense  />
...
for (let i = 1; i <= 100; i++) {
  ...
  export default {
    ...
    setup() {
      ...
      return {
        ...
        OptionSelections1model:ref(null),
        comment:ref(null),
        OptionSelections1:['Cell count', 'Viability/Apoptosis','Proliferation' ],
        pagination: {
    ...
    watch: {
      OptionSelections1model(newValue){
        this.savecellcomp(newValue)
      },
    ...
    methods: {
      ...
      // updateData(){
      //   const selections1=this.OptionSelections.toString()
      //   // console.log(selections1,"sdfsdf")
      //   const type_id1=this.id_row
      //   // console.log(type_id1)
      //   API("auth.updateImmunofluorescent_nc_one", { emission: selections1,type_id:type_id1});


      // },
      savecellcomp(selections){
        const selections1=selections.toString()
        const type_id1=this.id_row
        this.OptionSelections1model=selections1
        API("auth.updateImmunofluorescent_nc_cellcomp_one", { cellular_components: selections1,type_id:type_id1});

        // OptionSelections1model
      },
      ...
      API("auth.updateImmunofluorescent_comment_one", {
      ...
      async addRow() {
        ...
        const emission1 = this.OptionSelections.toString()
        ...
        if (this.editedIndex > -1) {
          Object.assign(oldRow, updatedRow) && API("auth.updateImmunofluorescent_nc", { id1: id1,type: type1,type_id:type_id1, location: location1});
        } else {
          if ( location1  ) {

            this.data.push(this.editedItem) && API("auth.create_obj_nc", { type: type1,type_id:type_id1, location: location1});
          }
          else {
            this.$q.notify({
      ...
      async deleteItem(item) {
        ...
        this.data.splice(index, 1) && await API('auth.deleteImmunofluorescent_nc', { id });
      },
      async editItem(item) {
  ...
  //   API('auth.Immunofluorescent_nc_detials_id',{key}).then(res => {
  //       this.data = res
  //       const estring=res[0].emission
  //       const emissionArray = estring.split(',');
  //       console.log(emissionArray);
  //       OptionSelections=emissionArray
  ...
  // },
    ...
    // API("auth.createArirports", { name:"DeliAir", latLong:"-873.45",noOfRunWay:"3",typesOfAircraft:"full",runWayConstType:"simple",AtcFacility:"fine",refuling:"ok" });
      ...
      key=this.id_row
      // console.log(key)
     await API('auth.Immunofluorescent_nc_detials_id',{key}).then(res => {
        this.data = res
        const estring=res[0].emission
        const ecell=res[0].cellular_components
        const c=res[0].comment
        this.comment =c;

        this.OptionSelections1model=ecell;
        // const emissionArray = estring.split(',');
        const emissionArray = estring ? estring.split(',') : [];
