<template>
    ...
    <div>
      <div class="q-pa-sm q-gutter-sm">
        <div class="text-center text-black text-h6">Nuclear Counterstain</div>
      ...
      <q-dialog v-model="show_dialog">
        <q-card style="width: 230px; max-width: 230px; background-color: #F5F5F5">
          <q-card-section>
            <div class="text-h6 text-center bg-grey-5">Add Nuclear Counterstain</div>
          </q-card-section>
          ...
          <q-card-section align="center">
            <div class="row text-center">
              ...
              <q-input
                ...
                placeholder="Nuclear Counterstain"
                stack-label
                :rules="[
      ...
      <q-dialog v-model="show_dialog_attributes">
        <q-card
          style="width: 700px; max-width: 700px; background-color: #F5F5F5"
        >
          <!-- <q-card-section>
  ...
  import immunofluorescentNc_attri from "src/components/admin/immunofluorescent/immunofluorescentNc_attri";
  ...
  export default {
    ...
    components: { immunofluorescentNc_attri },
    methods: {
      async addRow() {
        ...
        if (this.editedIndex > -1) {
          ...
          // this.data[editedIndex1] = updatedRow;
            API("auth.updateImmunofluorescent_nc", { id1: id1, type_id: type_id1 });
        } else {
          if (type_id1) {
            this.data.push(this.editedItem) ;
            // console.log("Added row:", this.editedItem); // Debugging line: Check the added row data

            await API("auth.create_obj_nc", { type_id: type_id1, type:type1 });
          } else {
            this.$q.notify({
      ...
      async deleteItem(item) {
        ...
        confirm("Are you sure you want to Delete this user?") &&
          ...
          (await API("auth.deleteImmunofluorescent_nc", { id }));
      },
      async editItem(item) {
