...
<!--
     ...
     <!-- basal /non basal start -->



<div v-if="selected.length > 0" class="q-mt-lg text-center">
    <q-btn
      label="Morphometric Heatmap"
      @click="dialogHeatmap"
      color="teal"
      unelevated
      icon="grid_view"
    ...
    <q-btn
      label="Basal / Non-Basal"
      @click="dialogHeatmap_basal"
      color="primary"
      unelevated
      icon="bubble_chart"
      class="q-mb-sm q-mr-lg"
  ...
  <q-card style="background-color: #F5F5F5; max-width: 2000px;width: 1000px;" class="q-dialog-plugin q-dialog-centered">
...
<q-dialog v-model="show_heatmap" maximized>
  <q-card style="background-color: #F5F5F5; width: 100vw; height: 100vh;">
...
export default {
  ...
  methods: {
    ...
    // },
async dialogHeatmap_positive() {
  try {
    const response1 = this.selected.map(item => ({
      // label: `${item.name} ${item.unit_harvest} ${item.time}`,
      label: `${item.name} | ${item.drug}`,
      experiment_id: item.files && item.files[0] ? item.files[0].experiment_id : null,
      file_name: item.files && item.files[0] ? item.files[0].file_name : null,
    }));

    const response1String = JSON.stringify(response1);
    console.log("positive payload =", response1);

    const res = await API('auth.getpositiveanalysi', response1String);
    console.log(res, "Positive Analysis Response");
    ...
    if (!Array.isArray(res) || res.length === 0) {
      this.$q.notify({
        color: 'red',
        message: 'No positive-cell data found.',
        icon: 'warning',
      });
      return;
    }

    const validPositiveRows = res.filter(item => {
      if (!item.positive) return false;

      try {
        const parsed = JSON.parse(item.positive);
        return Object.keys(parsed).length > 0 && parsed.status !== "failed";
      } catch (e) {
        return false;
      }
    });

    if (validPositiveRows.length === 0) {
      this.$q.notify({
        color: 'red',
        message: 'No positive-cell analysis found. Run positive analysis first.',
        icon: 'warning',
      });
      return;
    }

    const sendToServer = await API('home.getPositiveBarplot', validPositiveRows);
    ...
    if (sendToServer) {
      ...
      this.$q.notify({
        color: 'red',
        message: 'Failed to generate positive-cell bar plot.',
        icon: 'warning',
      });
    }

  } catch (error) {
    console.error("Error in dialogHeatmap_positive:", error);
    this.$q.notify({
      color: 'red',
      message: 'Error while generating positive-cell bar plot.',
      icon: 'error',
    });
  }
},
async dialogHeatmap_basal() {
  try {
    const response1 = this.selected.map(item => ({
      label: `${item.name} ${item.unit_harvest} ${item.time}`,
      experiment_id: item.files && item.files[0] ? item.files[0].experiment_id : null,
    ...
    const response1String = JSON.stringify(response1);
    console.log("basal payload =", response1);

    const res = await API('auth.getbasalanalysi', response1String);
    console.log(res, "Basal Analysis Response");
    ...
    if (!Array.isArray(res) || res.length === 0) {
      this.$q.notify({
        color: 'red',
        message: 'No valid basal/non-basal data found.',
        icon: 'warning',
      });
      return;
    }

    const validBasalRows = res.filter(item => {
      if (!item.basal) return false;

      try {
        const parsed = JSON.parse(item.basal);
    ...
    console.log("valid basal rows =", validBasalRows);

    if (validBasalRows.length === 0) {
      this.$q.notify({
        color: 'red',
        message: 'No basal/non-basal analysis found. Run basal analysis first.',
        icon: 'warning',
      });
      return;
    }

    const sendToServer = await API('home.getHeatmap_basal', validBasalRows);
    ...
    if (sendToServer) {
      ...
      this.$q.notify({
        color: 'red',
        message: 'Failed to generate basal/non-basal heatmap.',
        icon: 'warning',
      });
    }

  } catch (error) {
    console.error("Error in dialogHeatmap_basal:", error);
    this.$q.notify({
      color: 'red',
      message: 'Error while generating basal/non-basal heatmap.',
      icon: 'error',
    });
  }
},
    ...
    async dialogHeatmap() {
  try {
    // Map selected items to create a structured request payload
    const response1 = this.selected.map(item => ({
      label: `${item.name} ${item.unit_harvest} ${item.time}`,
      experiment_id: item.files && item.files[0] ? item.files[0].experiment_id : null,
      file_name: item.files && item.files[0] ? item.files[0].file_name : null,
    ...
    // API call for model analysis
    const res = await API('auth.getmodelanalysi', response1String);
    console.log(res, "Model Analysis Response");

    // Set table JSON if response is valid
    this.table_json = res;

    if (Array.isArray(res) && res.length > 0) {
      // API call to generate heatmap
      const sendToServer = await API('home.getHeatmap', res);
      ...
      if (sendToServer) {
        ...
        console.error("Heatmap generation failed: No image returned.");
        this.$q.notify({
          color: 'red',
          message: 'Failed to generate heatmap. Please try again later.',
          icon: 'warning',
        });
      }
    } else {
      console.error("Model analysis failed: Invalid or empty response.");
      this.$q.notify({
        color: 'red',
        message: 'No valid data found for heatmap generation.',
        icon: 'warning',
      });
    }
  } catch (error) {
    console.error("Error in dialogHeatmap:", error);
    this.$q.notify({
      color: 'red',
      message: 'An error occurred while processing the heatmap. Please try again.',
      icon: 'error',
    });
  }
},
