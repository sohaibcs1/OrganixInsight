...
<template v-slot:body-cell-time="props">
  <q-td :props="props">
    {{ props.row.unit_harvest }} {{ props.row.time }}
  </q-td>
</template>
...
export default {
 ...
 mounted(){
   // console.log(receivedphase,"phase")
     ...
     this.rows = a.map(row => ({
    ...row,
    treatment_group: row.treatment_group || "Treated"  // Set default for existing rows
  }));
     //Exp pass
     const exp_pass__propGlobal=[...this.exp_pass_combination_prop];
     this.exp_pass_assy=exp_pass__propGlobal

  this.normalizeTimesInPlace();
 ...
 components: {
   ...
   methods:{
    ...
    normalizeTimesInPlace() {
    // work directly on this.rows (or this.data), no cloning
    const rows = this.rows;  // or this.data

    // collect distinct, sorted time values (e.g. [2, 4])
    const timeOptions = [...new Set(rows.map(r => r.time))]
    ...
    for (const row of rows) {
      // only skip if the *key fields* are missing
      if (!row.name || !row.unit_harvest) {
        continue;
      }

      const groupKey = `${row.name}|${row.unit_harvest}`;

      if (!nameCounts[groupKey]) {
        nameCounts[groupKey] = 0;
      }

      const thisIndex = nameCounts[groupKey]; // 0, 1, 2, ...

      // cycle time: 2, 4, 2, 4, ...
      if (timeOptions.length > 0) {
        row.time = timeOptions[thisIndex % timeOptions.length];
      }

      // ensure unique uuid for *additional* duplicates only
      if (thisIndex > 0 && !row.uuid) {
        row.uuid = this.generateRandomUUID();
      }

      nameCounts[groupKey] += 1;
    }
  },
 ...
 if (Array.isArray(this.receivedantibody)) { // Ensure receivedantibody is an array
   ...
   generateRandomUUID() {
     ...
     return this.rows.map(row => `${row.time} ${row.unit_harvest}`);
        ...
        //  },
filteredRows() {
  const rows = this.rows; // same objects
  ...
  for (const row of rows) {
    // must have name, stroma, unit_harvest
    if (!row.name || !row.unit_harvest) continue;

    // include stroma in the grouping key
    const stromaKey = (row.stroma !== undefined && row.stroma !== null)
      ? row.stroma
      : "NULL";

    const key = `${row.name}|${stromaKey}|${row.unit_harvest}`;
    const idx = nameCounts[key] || 0;

    if (timeOptions.length > 0) {
      row.time = timeOptions[idx % timeOptions.length];
    }
...
filteredColumns() {
  ...
  // === ALWAYS FIRST: unit_harvest ===
  const allUnitHarvestNull = this.rows.every(r =>
    r.unit_harvest === null ||
    r.unit_harvest === "Null" ||
    r.unit_harvest === "" ||
    r.unit_harvest === undefined
  );



  // === Dynamic Columns EXCEPT treatment_group & unit_harvest ===
  for (const key in firstRow) {
    ...
    // hide column if all values are null/empty
    const allNull = this.rows.every(r =>
      r[key] === null ||
      r[key] === "Null" ||
      r[key] === "" ||
      r[key] === undefined
