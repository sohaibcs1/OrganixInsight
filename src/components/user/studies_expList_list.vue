<template>
    ...
    <div class="q-pa-sm q-gutter-sm text-black" v-if="expShow">
      ...
      <div class="q-ml-sm  q-gutter-md bg-grey-4">
        <div v-if="data.length > 0">
          ...
          <p><strong>Plasmid:</strong> {{ tableTitles.plasmid || 'N/A' }}</p>
 ...
 --><p v-if="showField(tableTitles.experiment_name)">
  <strong>Experiment Name:</strong> {{ tableTitles.experiment_name }}
</p>

<p v-if="showField(tableTitles.experiment_design_date)">
  <strong>Experiment Date:</strong> {{ tableTitles.experiment_design_date }}
</p>

<p v-if="showField(tableTitles.ex_type)">
  <strong>Experiment Type:</strong> {{ tableTitles.ex_type }}
</p>

<p v-if="showField(tableTitles.type)">
  <strong>Type:</strong> {{ tableTitles.type }}
</p>

<p v-if="showField(tableTitles.coCulture)">
  <strong>CoCulture:</strong> {{ tableTitles.coCulture }}
</p>

<p v-if="showField(tableTitles.costain)">
  <strong>Costain:</strong> {{ tableTitles.costain }}
</p>

<p v-if="showField(tableTitles.counterstain)">
  <strong>Counterstain:</strong> {{ tableTitles.counterstain }}
</p>

<p v-if="showField(tableTitles.virus)">
  <strong>Virus:</strong> {{ tableTitles.virus }}
</p>

<p v-if="showField(tableTitles.plasmid)">
  <strong>Plasmid:</strong> {{ tableTitles.plasmid }}
</p>

<p v-if="showField(tableTitles.primary)">
  <strong>Primary Antibody:</strong> {{ tableTitles.primary }}
</p>

<p v-if="showField(tableTitles.secondary)">
  <strong>Secondary Antibody:</strong> {{ tableTitles.secondary }}
</p>

<p v-if="showField(tableTitles.phase_info)">
  <strong>Phase Info:</strong> {{ tableTitles.phase_info }}
</p>

<p v-if="showField(tableTitles.magnification)">
  <strong>Magnification:</strong> {{ tableTitles.magnification }}
</p>

<p v-if="showField(tableTitles.em_exposure)">
  <strong>em exposure:</strong> {{ tableTitles.em_exposure }}
</p>

<p v-if="showField(tableTitles.unit_em_exposure)">
  <strong>unit_em_exposure:</strong> {{ tableTitles.unit_em_exposure }}
</p>

<p v-if="showField(tableTitles.voltage)">
  <strong>voltage:</strong> {{ tableTitles.voltage }}
</p>

<p v-if="showField(tableTitles.pulse_width)">
  <strong>pulse width:</strong> {{ tableTitles.pulse_width }}
</p>

<p v-if="showField(tableTitles.no_of_pluses)">
  <strong>no.of pluses:</strong> {{ tableTitles.no_of_pluses }}
</p>

<p v-if="showField(tableTitles.time_bt_pulses)">
  <strong>time between pulses:</strong> {{ tableTitles.time_bt_pulses }}
</p>

<p v-if="showField(tableTitles.capacitance)">
  <strong>capacitance:</strong> {{ tableTitles.capacitance }}
</p>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        { name: 'name', align: 'center', label: 'Biospecimen', field: 'name', sortable: false },
        { name: 'harvest_time', align: 'center', label: 'Harvest Time', field: 'harvest_time', sortable: false },
        { name: 'drug', align: 'center', label: 'Drug', field: 'drug', sortable: false },
        { name: 'concentration', align: 'center', label: 'Drug Concentration', field: 'concentration', sortable: false },
        { name: 'treatment_group', align: 'center', label: 'Condition', field: 'treatment_group', sortable: false },
  ...
  computed: {
    filteredRows() {
      return this.data.map(row => {
        ...
        rest.harvest_time = time && unit_harvest ? `${time}` : 'N/A';
  ...
  methods: {
    ...
    if (value === "" || value === "null" || value === "None") return false;
    return true;
  },
    setTableTitles(index) {
      if (this.data[index]) {
        const { experiment_name,experiment_design_date, ex_type, type, coCulture, costain, counterstain, virus,plasmid,primary, secondary, magnification, phase_info,em_exposure,unit_em_exposure,voltage,pulse_width,no_of_pluses,time_bt_pulses,capacitance } =
          this.data[index];
