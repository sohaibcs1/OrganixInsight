...
-->
  <div>
    <div class="q-pa-sm q-gutter-sm">
      ...
      <div class="text-h6 text-center text-black bg-grey-6 "> <q-icon name="query_stats" /> Analytics</div>
      ...
      <q-table title="Treats" :rows="data" :columns="columns" :dense="$q.screen.lt.md" row-key="name" max-width="100%"
        ...
        <template v-slot:body="props">
          <q-tr :props="props">
            ...
            <div><q-badge color="purple" v-if="props.row.name"> Name: {{ props.row.name }}</q-badge></div>
             <div><q-badge color="primary" v-if="props.row.counterstain"> counterstain: {{ props.row.counterstain }}</q-badge></div>
             <div><q-badge color="teal" v-if="props.row.primary"> Primary: {{ props.row.primary  }}</q-badge></div>
              <div><q-badge color="secondary" v-if="props.row.secondary"> Secondary: {{ props.row.secondary }}</q-badge></div>
              <div><q-badge color="warning" v-if="props.row.passage">  {{ (props.row.time && props.row.unit_harvest) ? (props.row.unit_harvest+ " "+  props.row.time ): 'N/A'}}</q-badge></div>


             <!-- <div><q-badge color="primary" v-if="props.row.experiment_design_date"> experiment design date: {{ props.row.experiment_design_date}}</q-badge></div>  -->


                <!-- <div><q-badge color="red" v-if="props.row.time"> Time: {{ props.row.time }} {{ props.row.unit_harvest }}</q-badge></div> -->

              <!-- <div><q-badge color="purple" v-if="props.row.passage"> passage: {{ props.row.passage }}</q-badge></div>
              ...
              <div><q-badge color="primary" v-if="props.row.counterstain"> counterstain: {{ props.row.counterstain }}</q-badge></div>
              <div><q-badge color="primary" v-if="props.row.drug"> drug: {{ props.row.drug }} {{  props.row.concentration }}{{ props.row.unit_drug }}</q-badge></div>
              <div><q-badge color="teal" v-if="props.row.primary"> Primary: {{ props.row.primary  }}</q-badge></div>
              <div><q-badge color="teal" v-if="props.row.secondary"> Secondary: {{ props.row.secondary }}</q-badge></div>
                <div><q-badge color="teal" v-if="props.row.treatment_group"> treatment group: {{ props.row.treatment_group }}</q-badge></div>  -->
            ...
            <q-td key="actions" :props="props">
              ...
              <div>
                  <!-- {{file.max_encrypt_image}} -->
                    ...
                    <q-img  :src="`${BASE_URL}/${file.max_encrypt_image}`"
                    spinner-color="white" style="height: 200px; width: 200px ;border: 1px solid #ccc;"/>

                    <q-tooltip>
                    {{ file.file_name }}
                  </q-tooltip>
                  <!-- {{ file.file_name }} -->
                  <div class="bg-blue text-subtitle1" @click="call_analysis(file,file.file_name)"> <q-icon name="query_stats" /> Analysis Details
    ...
    <q-dialog v-model="show_heatmap" >
        <q-card style="width: 1000px; max-width: 1000px; background-color: #F5F5F5">
