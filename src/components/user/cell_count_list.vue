  ...
  <div>
    <div class="q-pa-sm q-gutter-sm">
      ...
      <q-table  :rows="data" :columns="columns" :dense="$q.screen.lt.md" row-key="k-id" max-width="100%"
        ...
        class="q-table--dense compact-table" :pagination="pagination"  selection="multiple" v-model:selected="selected" >
      ...
      <template v-slot:body-cell-desc="props">
        <q-td key="desc" :props="props">
             ...
             <!-- <div><q-badge color="green" v-if="props.row.experiment_name"> experiment Name: {{ props.row.experiment_name}}</q-badge></div>  -->
              ...
              <q-badge   style="cursor: pointer;" color="secondary" v-if="props.row.name">{{ props.row.name }}</q-badge>
                <q-badge style="cursor: pointer;" color="red" v-if="props.row.time"> {{ props.row.time }} {{ props.row.unit_harvest }}</q-badge>
                <!-- <q-badge color="warning" v-if="props.row.type"> type: {{ props.row.type }}</q-badge> -->
                <q-badge  style="cursor: pointer;"  color="accent" v-if="props.row.counterstain"> counterstain: {{ props.row.counterstain }}</q-badge>
                <q-badge  style="cursor: pointer;"  color="dark" v-if="props.row.drug"> drug: {{ props.row.drug }} {{  props.row.concentration }}{{ props.row.unit_drug }}</q-badge>
                <q-badge  style="cursor: pointer;"  color="positive" v-if="props.row.primary"> Primary: {{ props.row.primary  }}</q-badge>
                <q-badge  style="cursor: pointer;"  color="teal" v-if="props.row.secondary"> Secondary: {{ props.row.secondary }}</q-badge>
        ...
        <template v-slot:body-cell-actions="props">
                ...
                <div v-if="props.row.files && props.row.files[0] && props.row.files[0].middle_encrypt_image">
                <q-img  :src="`${BASE_URL}/${props.row.files[0].middle_encrypt_image}`"
                spinner-color="white" style="height: 50px; width: 50px ;border: 1px solid #ccc;"/>
...
<!--
      ...
      <div class="text-center q-mt-sm q-mb-sm" v-if="selected.length > 0"> <q-btn @click="dialogHeatmap" color="teal"> Proceed</q-btn></div>
  ...
  <q-card style="background-color: #F5F5F5; max-width: 2000px;width: 1500px;" class="q-dialog-plugin q-dialog-centered">
    ...
    <q-card-section class="text-center">
      ...
      <!-- <div class="text-weight-thin">Cell Count</div> -->
      <q-img
    ...
    <!-- <q-btn color="primary" icon-right="download" label="Download Cell Count CSV"  class="q-mr-sm" @click="downloadCSV"/>
    <q-btn color="purple" icon-right="download" label="Download Protein CSV" @click="downloadProteinCSV" /> -->

    <div class="row justify-center items-center q-gutter-sm q-mt-md q-mb-md">
  <q-btn
    color="primary"
    icon-right="download"
    label="Download Cell Count CSV"
    @click="downloadCSV"
  />
  ...
  <q-btn
    color="purple"
    icon-right="download"
    label="Download Protein CSV"
    @click="downloadProteinCSV"
  />
</div>
...
async downloadCSV () {
  try {
    // 1) Collect selected experiment IDs
    const experimentIds = this.selected
      .map(item => item?.files?.[0]?.experiment_id ?? item?.experiment_id ?? item?.uuid?.toString?.())
      .filter(Boolean)
      .map(String)

    if (!experimentIds.length) {
      this.$q.notify({ message: 'No experiment IDs selected.', color: 'negative', position: 'top' })
      return
    }

    // 2) Build a meta lookup from the selected items: expId -> {name, drug, concentration}
    const metaByExpId = new Map()
    for (const item of this.selected) {
      const expId = (item?.files?.[0]?.experiment_id ?? item?.experiment_id ?? item?.uuid?.toString?.()) ?? null
      if (!expId) continue
      ...
      metaByExpId.set(String(expId), { name, drug, concentration })
    }

    // 3) Fetch rows from backend
    const rows = await API('auth.get2dmodel', { arr_experiment_id: experimentIds })
    // rows like: [{ experiment_id:'787', file_name:'B2_P3.tif', cell_count:24, ... }]

    if (!Array.isArray(rows) || rows.length === 0) {
      this.$q.notify({ message: 'No rows returned for selected experiments.', color: 'negative', position: 'top' })
      return
    }

    // 4) Build CSV (merge backend with our meta map)
    const header = ['experiment_id', 'file_name', 'cell_count', 'name', 'drug', 'concentration']
    const csvLine = arr => arr.map(v => `"${String(v ?? '').replace(/"/g, '""')}"`).join(',')

    const body = rows.map(r => {
      const expId = String(r.experiment_id ?? '')
      const meta = metaByExpId.get(expId) || {}

      const name = r.name ?? meta.name ?? ''
      const drug = r.drug ?? meta.drug ?? ''
      const concentration = r.concentration ?? meta.concentration ?? ''
    ...
    const csv = [csvLine(header), ...body].join('\r\n')

    // 5) Download
    const dateStr = new Date().toISOString().slice(0, 10)
    const ok = exportFile(`cell_counts_${dateStr}.csv`, csv, 'text/csv')

    if (ok === true) {
      this.$q.notify({ message: 'CSV downloaded.', color: 'positive', position: 'top' })
    } else {
      this.$q.notify({ message: 'Browser blocked the download.', color: 'negative', position: 'top' })
    }
  } catch (e) {
    console.error(e)
    this.$q.notify({ message: 'Failed to generate CSV.', color: 'negative', position: 'top' })
  }
}

,
async downloadProteinCSV () {
  try {
    // 1) Collect selected experiment IDs
    const experimentIds = this.selected
      .map(item => item?.files?.[0]?.experiment_id ?? item?.experiment_id ?? item?.uuid?.toString?.())
    ...
    if (!experimentIds.length) {
      this.$q.notify({
        message: 'No experiment IDs selected.',
        color: 'negative',
        position: 'top'
      })
      return
    ...
    // 2) Fetch rows from backend
    const rows = await API('auth.get2dmodelprot', {
      arr_experiment_id: experimentIds
    })

    if (!Array.isArray(rows) || rows.length === 0) {
      this.$q.notify({
        message: 'No rows returned for selected experiments.',
        color: 'negative',
        position: 'top'
      })
      return
    ...
    const dateStr = new Date().toISOString().slice(0, 10)
    const ok = exportFile(
      `protein_intensity_${dateStr}.csv`,
      csv,
      'text/csv'
    )
    ...
    if (ok === true) {
      this.$q.notify({
        message: 'Protein CSV downloaded.',
        color: 'positive',
        position: 'top'
      })
    } else {
    ...
    console.error(e)
    this.$q.notify({
      message: 'Failed to generate Protein CSV.',
      color: 'negative',
      position: 'top'
    })
  }
...
//     // 1) Collect selected experiment IDs
//     const experimentIds = this.selected
//       .map(item => item?.files?.[0]?.experiment_id ?? (item?.uuid?.toString?.() ?? ''))
//       .filter(Boolean)
...
//     // 2) Fetch rows from backend
//     const rows = await API('auth.get2dmodel', { arr_experiment_id: experimentIds })
//     // rows expected like: [{ experiment_id:'787', file_name:'B2_P3.tif', cell_count:24 }, ...]
...
//     // 3) Build CSV
//     const header = ['experiment_id', 'file_name', 'cell_count']
//     const csvLine = arr => arr.map(v => `"${String(v ?? '').replace(/"/g, '""')}"`).join(',')
//     const body = rows.map(r => csvLine([r.experiment_id, r.file_name, r.cell_count]))
//     const csv = [csvLine(header), ...body].join('\r\n')
...
// downloadCSV () {
//   try {
//     // Build rows from selected items:
//     // Prefer files[0].experiment_id (matches your dialogHeatmap usage),
//     // fall back to row.uuid if experiment_id is missing.
//     const header = ['experiment_id']
//     const rows = this.selected.map(item => {
//       const f = item?.files?.[0]
//       const expId = f?.experiment_id ?? (item?.uuid?.toString?.() ?? '')
//       return [expId]
//     })

//     // Guard: if no IDs resolved, notify
//     if (!rows.length || rows.every(r => !r[0])) {
//       this.$q.notify({
//         message: 'No experiment IDs found in the selected rows.',
//         color: 'negative',
//         position: 'top'
//       })
//       return
//     }
//     console.log(rows,"kkkkkk")
//     // CSV encode
//     const csvLine = arr =>
//       arr.map(v => `"${String(v ?? '').replace(/"/g, '""')}"`).join(',')
...
//     // Filename like selected_experiments_2025-09-17.csv
//     const dateStr = new Date().toISOString().slice(0, 10)
//     const status = exportFile(
//       `selected_experiments_${dateStr}.csv`,
//       csv,
//       'text/csv'
//     )
...
//     if (status !== true) {
//       this.$q.notify({
//         message: 'Browser denied the download…',
//         color: 'negative',
//         position: 'top'
//       })
//     } else {
...
,
  ...
  const data = this.selected
    .map(item => {
      ...
      return [
        f.experiment_id,
      `${item.drug} ${item.concentration} ${item.unit_drug ?? item.unit ?? ""} | ${item.name}`.trim(),
        item.treatment_group ?? f.treatment_group ?? ""
      ];
    })
  ...
  // send to server (adjust the key name if your API expects a different one)
  const sendToServer = await API('home.getCellcount', { data });
  console.log(sendToServer, "ServerResponse");
  this.base64Image=sendToServer.image_base64
  this.show_heatmap=true;
},
